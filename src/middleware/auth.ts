...
export const requireAuth = createMiddleware<AppEnv>(async (c, next) => {
  ...
  // Only a client-credentials token (sub_type 'service', `sub` an app-service id)
  // may lack a user row. Anything else -- 'user', a missing claim, a value we
  // never mint -- stays valid only while its user exists, so deleting an account
  // revokes its in-flight access tokens. Allow-list: an unknown type fails closed.
  if (claims.sub_type !== 'service' && !user) {
    throw AppError.of('invalid_token')
  }
  c.set('user', {
    id: claims.sub,
