...
field. This depends on the password grant, so unless `ALLOW_PASSWORD_GRANT=true`
(it is off by default) `POST /users/guest` refuses (404
`guest_accounts_disabled`) and existing guests cannot sign in (see
